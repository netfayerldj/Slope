...
namespace ClassLibrary
{
    ...
    public class Class
    {
        ...
        static public void spx2()
        {
            ...
            if (isDrillSlope)
                myDSS.DrawShortLine3(drillPt, bottomCurve, step1, myColor, top1);
            else
                myDSS.DrawShortLine2(topCurve, bottomCurve, step2, myColor, top2);
    ...
    public class DrawSlope
    {
        ...
        public void DrawShortLine2(Curve topCurve, Curve bottomCurve, double step, int colorIndex, bool top)
        {
            ...
            while (tmpDis <= topCurveLen)
            {
                ...
                v1 = myCurve1.GetFirstDerivative(pt1).GetNormal();
        ...
        public void DrawShortLine3(Point3d pt, Curve myCurve, double step, int colorIndex, bool top)
        {
            DotNetArxLH.LhDraw.LhLine lhline = new DotNetArxLH.LhDraw.LhLine();
            ...
            {
                tmpPt = myCurve.GetClosestPointTo(myCurve.GetPointAtDist(tmpDis), false);


                if (isWhole)
                {
                    tmpLine = new Line(tmpPt, pt);
                    tmpLine.ColorIndex = colorIndex;
                    myLines.Add(tmpLine);
                    isWhole = false;
                }
                ...
                {
                    if (top)
                        tmpLine = new Line(pt, new Point3d((tmpPt.X + pt.X) / 2, (tmpPt.Y + pt.Y) / 2, 0));
                    else
                        tmpLine = new Line(tmpPt, new Point3d((tmpPt.X + pt.X) / 2, (tmpPt.Y + pt.Y) / 2, 0));
                    tmpLine.ColorIndex = colorIndex;
                    myLines.Add(tmpLine);
                    isWhole = true;
                }
